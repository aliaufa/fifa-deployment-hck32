...
def run():
    ...
    # element gambar
    st.image('f23-featured-image-ea-play-trial.jpg.adapt.crop191x100.628p.jpg',
    ...
    st.markdown('''
                ...
                jumlah pemain sepakbola pada tahun 2021 kurang lebih sebanyak 130.000 pemain. 
                Namun, dalam dataset yang digunakan pada kali ini, hanya mencakup 20.000 pemain 
                saja.
                
                Project kali ini bertujuan untuk memprediksi rating pemain FIFA 2022 
                sehingga semua pemain sepak bola profesional dapat diketahui ratingnya 
                dan tidak menutup kemungkinan untuk lahirnya talenta/wonderkid baru.
                
                Project ini akan dibuat menggunakan algoritma Linear Regresison dan akan 
                dievaluasi dengan menggunakan metrics **MAE (Mean Absolute Error)**.''')
    ...
    # insight
    st.markdown('''Terlihat dari Histogram Plot diatas bahwa `Rating` 
                memiliki distribusi normal dengan mayoritas data berada 
                pada rentang `60` hingga `70`.''')
    ...
    # user input
    option = st.selectbox('Pilih atribut untuk divisualisasikan' ,
                        options = total_cols)
    ...
    # # plotly
    # fig = px.histogram(data, x=option, nbins = 50)
    # st.plotly_chart(fig)
